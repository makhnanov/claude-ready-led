...
scripts/led.sh find     # العثور على اللوحة في الشبكة وحفظ عنوانها
scripts/led.sh where    # طباعة العنوان المستخدم حاليًا
```

يُؤخذ العنوان من `CLAUDE_LED_URL` (البيئة أو `~/.claude-led.conf`)، وإلا فمن عنوان IP
المحفوظ في `~/.cache/claude-led/ip`؛ يكتبه الأمر `find`، ويحدّثه السكربت من تلقاء نفسه
ما إن يتوقف العنوان المحفوظ عن الاستجابة (مرة في الدقيقة على الأكثر، انظر
`CLAUDE_LED_SCAN_COOLDOWN`). وهكذا يصلح تغيّر عقد DHCP نفسه بنفسه، دون تعديل أي إعداد.

حين يُستدعى الأمر من خطاف أو من اختصار لوحة مفاتيح — أي حيثما لا يكون stdout طرفية —
ينفصل الأمر الذي يشغّل المصباح فورًا وينجز عمل الشبكة في الخلفية: يعود خلال أجزاء من
الألف من الثانية ويخرج دائمًا بالرمز 0، وهذا ما يجعل تعليقه على خطاف آمنًا. أما إذا
كتبته بيدك في الطرفية فيبقى في المقدمة ويطبع ردّ اللوحة كما كان. تفرض
`CLAUDE_LED_WAIT=1` الانتظار دائمًا، و`CLAUDE_LED_WAIT=0` الانفصال دائمًا.

لاحظ أن `claude-led.local` لا يُترجَم إلى عنوان أبدًا في مسار الخطاف: استعلام mDNS
الفاشل يكلّف خمس ثوانٍ كاملة، وهو بالضبط ما كان يفجّر مهلة الخطاف كلما كانت اللوحة
مطفأة. ترجمة الاسم لا تحدث إلا داخل `find`. وإذا صمت العنوان المحفوظ، يمسح `find`
أيضًا شبكتك المحلية `/24` — المنفذ 80 وحده، ومحاولة واحدة لكل مضيف — فإن كانت شبكتك لا
تحتمل ذلك، فثبّت العنوان عبر `CLAUDE_LED_URL`.
