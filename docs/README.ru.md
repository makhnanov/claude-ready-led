...
scripts/led.sh find     # найти плату в сети и запомнить её адрес
scripts/led.sh where    # показать адрес, который используется сейчас
```

Адрес берётся из `CLAUDE_LED_URL` (окружение или `~/.claude-led.conf`), иначе из кэша
`~/.cache/claude-led/ip` — его пишет `find`, и скрипт сам обновляет кэш, как только
запомненный адрес перестаёт отвечать (не чаще раза в минуту, см.
`CLAUDE_LED_SCAN_COOLDOWN`). Смена DHCP-аренды лечится сама, конфиги править не надо.

Вызванная из хука или с горячей клавиши — то есть везде, где stdout не терминал —
команда-действие мгновенно отцепляется и делает сетевую работу в фоне: возвращается за
миллисекунды и всегда с кодом 0, поэтому её безопасно вешать на хуки. Набранная руками
в терминале, она по-прежнему ждёт и печатает ответ платы. `CLAUDE_LED_WAIT=1`
заставляет ждать всегда, `CLAUDE_LED_WAIT=0` — не ждать никогда.

Имя `claude-led.local` в пути хука не резолвится вообще: неудачный mDNS-запрос стоит
целых 5 секунд, и раньше именно он ронял хук по таймауту, когда плата была выключена.
Разрешение имени происходит только внутри `find`. Если закэшированный адрес молчит,
`find` заодно проходит по локальной `/24` — только порт 80, по одному запросу на хост;
в сети, где такое нежелательно, лучше прибить адрес через `CLAUDE_LED_URL`.
