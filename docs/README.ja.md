...
アドレスは `CLAUDE_LED_URL`、次に `~/.claude-led.conf`、最後に
`http://claude-led.local` の順で決まります。このスクリプトは必ず終了コード 0 で終わり、
2 秒以上ブロックしません。だからこそフックに掛けても安全です。電源が入っていない、
あるいは届かないボードが Claude Code を遅くしたり壊したりすることはありません。
